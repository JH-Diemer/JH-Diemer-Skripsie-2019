...
for k in range(40): #loop for training runs 
    ...
    path = "/home/skripsie/Work/CSVFiles/CliffWalking_DQN_0.%d.csv" % k
    file = open(path, 'w')
...
#    policy_net = torch.load("NN_DQN_train")
#    policy_net.eval()
#    target_net = torch.load("NN_DQN_train")
#    target_net.eval()
...
#    
    ...
    for i in range(814): #loop for episodes
...
#                    print("oye")
        ...
        if i % TARGET_UPDATE == 0:
            target_net.load_state_dict(policy_net.state_dict())
#            torch.save(target_net, "NN_DQN%d" % k)
        steps_done += 1  
        print()        
        print(R_total)  
