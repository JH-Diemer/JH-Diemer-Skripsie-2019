...
#    root.update()
    ...
    path = "/home/skripsie/Work/CSVFiles/CliffWalking_Double-Q-learning_coord_%d.csv" % k
    file = open(path, 'w')
    thewriter = csv.writer(file)
    thewriter.writerow(['Reward'])
    
    q_table = build_q_table(ACTIONS, first_call=True)
    q1_table = build_q_table(ACTIONS, first_call=True)
    q2_table = build_q_table(ACTIONS, first_call=True)
    ...
    #------------------- Determine the initial S --------------------    
        ...
        S = "%d:%d" % (int(obs[u'XPos']), int(obs[u'ZPos'])) 
    #------------------- END Determine the initial S -------------------- 
        
        R_total = 0 #Sum of Rewards
        
        q_table = build_q_table(ACTIONS, S, q_table)
        q1_table = build_q_table(ACTIONS, S, q1_table)
        q2_table = build_q_table(ACTIONS, S, q2_table)
        ...
        while world_state.is_mission_running: 
            ...
            if world_state.is_mission_running and len(world_state.observations)>0 and not world_state.observations[-1].text=="{}":
                ...
                S_ = "%d:%d" % (int(obs[u'XPos']), int(obs[u'ZPos'])) #where did the agent land?
                q_table = build_q_table(ACTIONS, S_, q_table)
                q1_table = build_q_table(ACTIONS, S_, q1_table)
                q2_table = build_q_table(ACTIONS, S_, q2_table)
    ...
    #------------------- Q-update for each Step (based on the algorithm used) --------------------            
            if np.random.choice([True, False]):
                q1_predict = q1_table.loc[S, A]
                if world_state.is_mission_running: #update Q value
                    q1_target = R + GAMMA * q2_table.loc[S_, q1_table.loc[S_, :].idxmax(axis =1)]
                if not world_state.is_mission_running: #update Q value from terminal state
                    q1_target = R
                q1_table.loc[S, A] += ALPHA * (q1_target - q1_predict)
            else:
                q2_predict = q2_table.loc[S, A]
                if world_state.is_mission_running: #update Q value
                    q2_target = R + GAMMA * q1_table.loc[S_, q2_table.loc[S_, :].idxmax(axis =1)]
                if not world_state.is_mission_running: #update Q value from terminal state
                    q2_target = R
                q2_table.loc[S, A] += ALPHA * (q2_target - q2_predict)
